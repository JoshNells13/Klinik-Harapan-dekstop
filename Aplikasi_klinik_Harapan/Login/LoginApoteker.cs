...
{
    public partial class LoginApoteker : Form
    {
        ...
        private void login_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT * FROM Apoteker WHERE Email = @email AND PasswordHash = @password", con);
                ...
                if (rd.Read())
                {
                    string status = rd["StatusPraktik"].ToString();  // Pastikan kolomnya di database namanya 'Status'

                    if (status == "Aktif")
                    {
                        DashboardApoteker ke_Menu = new DashboardApoteker();
        ...
        private void LoginApoteker_Load(object sender, EventArgs e)
        {
