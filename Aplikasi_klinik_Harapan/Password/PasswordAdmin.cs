...
{
    public partial class PasswordAdmin : Form
    {
        ...
        string.IsNullOrWhiteSpace(txtconfirm.Text))
            ...
            using (SqlConnection con = conn.GetConn())
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM Admin WHERE Email = @Email AND PasswordHash = @Password", con))
                {
                    ...
                    using (SqlDataReader rd = cmd.ExecuteReader())
                    {
                        if (rd.Read()) // Jika data ditemukan (password lama cocok)
                        {
                            ...
                            using (SqlCommand cmd2 = new SqlCommand("UPDATE Admin SET PasswordHash = @NewPassword WHERE Email = @Email", con))
                            {
