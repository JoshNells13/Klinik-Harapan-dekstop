...
namespace Aplikasi_klinik_Harapan.Password
{
    ...
    public partial class PasswordApoteker : Form
    {
        ...
        public PasswordApoteker()
        {
            InitializeComponent();
        }

        private void PasswordApoteker_Load(object sender, EventArgs e)
        {
       ...
       string.IsNullOrWhiteSpace(txtconfirm.Text))
            ...
            using (SqlConnection con = conn.GetConn())
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM Apoteker WHERE Email = @Email AND PasswordHash = @Password", con))
                {
                    ...
                    using (SqlDataReader rd = cmd.ExecuteReader())
                    {
                        if (rd.Read()) // Jika data ditemukan (password lama cocok)
                        {
                            ...
                            using (SqlCommand cmd2 = new SqlCommand("UPDATE Apoteker SET PasswordHash = @NewPassword WHERE Email = @Email", con))
                            {
