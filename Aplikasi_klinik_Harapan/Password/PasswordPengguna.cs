...
{
    public partial class PasswordPengguna : Form
    {
        ...
        private void PasswordPengguna_Load(object sender, EventArgs e)
        {
        ...
        string.IsNullOrWhiteSpace(txtconfirm.Text))
            ...
            using (SqlConnection con = conn.GetConn())
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM Pasien WHERE Email = @Email AND PasswordHash = @Password", con))
                {
                    cmd.Parameters.AddWithValue("@Email", emailkedua);
                    ...
                    using (SqlDataReader rd = cmd.ExecuteReader())
                    {
                        if (rd.Read()) // Jika data ditemukan (password lama cocok)
                        {
                            ...
                            using (SqlCommand cmd2 = new SqlCommand("UPDATE Pasien SET PasswordHash = @NewPassword WHERE Email = @Email", con))
                            {
                                cmd2.Parameters.AddWithValue("@NewPassword", txtnewpw.Text);
                                cmd2.Parameters.AddWithValue("@Email", emailkedua);
